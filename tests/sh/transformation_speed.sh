...
test_LargeCVS1_parse2XML_addcols_xslt() {
    time p2x $P2XOPTS --indent -m -p ../../examples/configs/csv -o res.xml $TMP/large.csv
...
test_LargeCVS1_parse2XML_Y_addcols_xslt() {
    time p2x $P2XOPTS --indent -m --output-mode=y -p ../../examples/configs/csv -o res.xml $TMP/large.csv
    assertEquals "P2X should not fail in this case" 0 $?
    sz2=$(ls -l res.xml | cut -d " " -f 5)
    ratio=$(( 1.0 * $sz2 / $sz1 ))
    echo "Output size ratio: $ratio"
...
test_LargeCVS1_parse2JSON_addcols_py() {
    time p2x $P2XOPTS --indent -m -J -p ../../examples/configs/csv -o res.json $TMP/large.csv
    assertEquals "P2X should not fail in this case" 0 $?
